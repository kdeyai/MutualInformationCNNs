...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        saved_labelixs = {}
        soft = []
        for j in model.input_data:
                soft.append(torch.argmax(model.layer_activations[j][5], dim = 1))
        
        softoutput = torch.cat(soft).detach().cpu()
        for i in range(10):
            saved_labelixs[i] = np.squeeze(softoutput == i)
